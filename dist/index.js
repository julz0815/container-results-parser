...
const stripAnsi = __nccwpck_require__(494);
const isFullwidthCodePoint = __nccwpck_require__(634);
const emojiRegex = __nccwpck_require__(27);
...
const ansiRegex = __nccwpck_require__(794);
...
const stringWidth = __nccwpck_require__(469);
const stripAnsi = __nccwpck_require__(494);
...
const yargs_1 = __importDefault(__nccwpck_require__(176));
const helpers_1 = __nccwpck_require__(98);
const child_process_1 = __nccwpck_require__(81);
const path = __importStar(__nccwpck_require__(17));
const os = __importStar(__nccwpck_require__(37));
function getDockerInspect(imageName) {
    try {
        const output = (0, child_process_1.execSync)(`docker inspect --format '{{json .}}' ${imageName}`, { encoding: 'utf8' });
...
function getDockerHistory(imageName) {
    try {
        const output = (0, child_process_1.execSync)(`docker history --no-trunc --format '{{.ID}}|{{.CreatedAt}}|{{.CreatedBy}}|{{.Size}}|{{.Comment}}' ${imageName}`, { encoding: 'utf8' });
...
function downloadAndExtractImage(imageName, localTar) {
    ...
    try {
        ...
        if (localTar) {
            ...
            (0, child_process_1.execSync)(`docker save ${imageName} -o ${tarPath}`);
        }
        console.log('Extracting image...');
        (0, child_process_1.execSync)(`tar -xf ${tarPath} -C ${tempDir}`);
...
function getImageConfig(imageDir) {
...
function getLayerHistory(layerId, imageDir, imageConfig) {
    try {
        let layerIndex = 0;
...
function main() {
    ...
    try {
        ...
        for (const [layerID, layer] of applicationLayers) {
            ...
            if (dockerfileInstructions) {
                const matchingInstruction = dockerfileInstructions.find(instruction => { var _a; return (_a = layer.created_by) === null || _a === void 0 ? void 0 : _a.includes(instruction.arguments); });
...
const stringWidth = __nccwpck_require__(469);
const stripAnsi = __nccwpck_require__(494);
const wrap = __nccwpck_require__(679);
function ui(opts) {
    return cliui(opts, {
        stringWidth,
        stripAnsi,
...
const {Yargs, processArgv} = __nccwpck_require__(9);
...
function Argv(processArgs, cwd) {
  const argv = Yargs(processArgs, cwd, __nccwpck_require__(907));
  singletonify(argv);
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(__nccwpck_require__.s = 925);
